# ==============================================================================
# BTC VOLATILITY FORECASTING WITH CAUSAL CEEMDAN FEATURES — BINANCE 5m — CONTROLS v2
...
#   - log HAR-RV-J + smearing (positive by construction)
#   - HAR-Ridge (regularized HAR-only feature benchmark)
#   - CEEMDAN-Ridge (HAR features + causal CEEMDAN features)
#   - GARCH_CAL: trivial rolling multiplicative GARCH calibration
#   - GARCH_HAR_CORR: GARCH residual correction using HAR state only
#   - GARCH_CEEMDAN_CORR: GARCH residual correction using CEEMDAN only
#   - GARCH_HAR_CEEMDAN_CORR: nested HAR + CEEMDAN residual correction
#
# PRIMARY IDENTIFICATION TEST:
#
#   Does GARCH_HAR_CEEMDAN_CORR beat GARCH_HAR_CORR?
#
# If not, CEEMDAN has not demonstrated incremental information beyond an
# ordinary multiscale HAR state description.
#
# SECOND IDENTIFICATION TEST:
#
#   Does GARCH_CEEMDAN_CORR beat GARCH_CAL?
#
# If not, the apparent "CEEMDAN improvement" is explainable by a trivial
# rolling multiplicative calibration of GARCH.
#
# Horizons:
...
cfg <- list(
  ...
  output_dir = "results/btc_ceemdan_binance_controls_v2"
)
...
forecast_har_log_rvj_smear_direct <- function(
    train_rv,
    train_jump,
    h,
    cfg
  ...
  # Positive-by-construction specification.
  # RV terms are logged. Jump terms use log1p because jumps are often exactly 0.
  df$ly <- log(
    pmax(df$y, cfg$rv_floor)
  )
  df$ld <- log(
  ...
  nd <- data.frame(
    ...
    ljw = log1p(
      pmax(
        mean(tail(train_jump, cfg$har_week)),
        0
      )
    ),
    ljm = log1p(
      pmax(
        mean(tail(train_jump, cfg$har_month)),
        0
      )
    )
  )
...
# ==============================================================================
# 8B. SIMPLE GARCH CALIBRATION CONTROL
# ==============================================================================

garch_calibration_predict <- function(
    train_df,
    target_name,
    garch_name,
    current_garch,
  ...
  # For multiplicative forecast c * GARCH, QLIKE is minimized in sample by:
  #
  #     c* = mean(actual / GARCH)
  #
  # This is also equivalent to an intercept-only log residual model with a
  # Duan-style smearing correction:
  #
  # log(actual) - log(GARCH) = alpha + e
  #
  # c = exp(alpha) * mean(exp(e)) = mean(actual/GARCH)
  ratio <- actual / garch
  ...
  raw <- current_garch * factor

  # Use the same train-only log guardrail as the ridge models.
  y_log <- log(actual)

  guarded <- clip_log_prediction(
    log(
      pmax(raw, cfg$forecast_floor)
    ),
    y_log,
    cfg
  )
...
for (oo in seq_along(candidate_oos_idx)) {
  ...
  for (h in cfg$horizons) {
    ...
    har_log_rvj_fc <- if (has_jump_data) {
      forecast_har_log_rvj_smear_direct(
        train_rv,
        train_jump,
        h,
        cfg
    ...
    # --------------------- GARCH calibration/control ladder --------------------
    #
    # 0) GARCH_CAL:
    #    trivial rolling multiplicative calibration, no predictors.
    #
    # 1) GARCH_HAR_CORR:
    #    predicts GARCH log forecast error using only HAR state.
    #
    # 2) GARCH_CEEMDAN_CORR:
    #    predicts GARCH log forecast error using only CEEMDAN state.
    #
    # 3) GARCH_HAR_CEEMDAN_CORR:
    #    nested model containing HAR + CEEMDAN.
    #
    # The clean incremental CEEMDAN test is (3) versus (1).

    garch_cal <- garch_calibration_predict(
      train_df = hist_store,
      target_name = target_name,
      garch_name = garch_name,
      current_garch = garch_fc,
      cfg = cfg
    )

    garch_har <- ridge_fit_predict(
      train_df = hist_store,
      current_row = current_store,
      feature_names = har_ridge_features,
      target_name = target_name,
    ...
    garch_har_ce <- ridge_fit_predict(
      train_df = hist_store,
      current_row = current_store,
      feature_names = ceemdan_ridge_features,
      target_name = target_name,
    ...
    model_values <- list(
      ...
      GARCH_CAL = list(
        forecast = garch_cal$forecast,
        raw = garch_cal$raw_forecast,
        lambda = NA_real_,
        clipped = garch_cal$clipped,
        ridge_n = garch_cal$n_train,
        n_features = 0L,
        cv_qlike = NA_real_,
        calibration_factor = garch_cal$factor
      ),
      GARCH_HAR_CORR = list(
        forecast = garch_har$forecast,
        raw = garch_har$raw_forecast,
        lambda = garch_har$lambda,
        clipped = garch_har$clipped,
        ridge_n = garch_har$n_train,
        n_features = garch_har$n_features,
        cv_qlike = garch_har$cv_qlike,
        calibration_factor = NA_real_
      ),
      ...
      GARCH_HAR_CEEMDAN_CORR = list(
        forecast = garch_har_ce$forecast,
        raw = garch_har_ce$raw_forecast,
        lambda = garch_har_ce$lambda,
        clipped = garch_har_ce$clipped,
        ridge_n = garch_har_ce$n_train,
        n_features = garch_har_ce$n_features,
        cv_qlike = garch_har_ce$cv_qlike,
        calibration_factor = NA_real_
      )
    )

    if (has_jump_data) {
      model_values$HAR_LOG_RVJ_SMEAR <- list(
        forecast = har_log_rvj_fc,
        raw = har_log_rvj_fc,
        lambda = NA_real_,
        clipped = FALSE,
        ridge_n = NA_integer_,
    ...
    for (model_name in names(model_values)) {
      ...
      forecast_rows[[counter]] <- data.frame(
        ...
        calibration_factor = if (
          is.null(z$calibration_factor)
        ) {
          NA_real_
        } else {
          z$calibration_factor
        },
        garch_multiplier = if (
          is.finite(garch_fc) &&
            garch_fc > 0 &&
...
# ==============================================================================
# 11B. CORRECTION / SHRINKAGE DIAGNOSTICS
# ==============================================================================
...
correction_models <- c(
  ...
  "GARCH_HAR_CORR",
  "GARCH_CEEMDAN_CORR",
  "GARCH_HAR_CEEMDAN_CORR"
)

corr_dat <- forecasts[
  forecasts$model %in% correction_models,
  ,
  drop = FALSE
]
...
correction_diagnostics <- do.call(
  rbind,
  lapply(
    corr_split,
    function(df) {
      mult <- df$garch_multiplier[
        is.finite(df$garch_multiplier)
      ]
      ...
      data.frame(
        ...
        pct_lambda_at_grid_max = if (
          length(lam) > 0L
        ) {
          mean(
            abs(lam - max_lambda) <
              1e-12 * max(1, max_lambda)
          )
        } else {
          NA_real_
        },
...
rownames(correction_diagnostics) <- NULL
...
comparison_pairs <- list(
  # Direct model: does CEEMDAN add to HAR?
  c(
    "CEEMDAN_RIDGE",
    "HAR_RIDGE"
  ),

  # How much of GARCH performance is fixed by trivial calibration?
  c(
    "GARCH_CAL",
    "GARCH_11"
  ),

  # Do dynamic residual corrections beat trivial calibration?
  c(
    "GARCH_HAR_CORR",
    "GARCH_CAL"
  ),
  ...
  # CEEMDAN-only versus ordinary HAR-only state correction.
  c(
    "GARCH_CEEMDAN_CORR",
    "GARCH_HAR_CORR"
  ),
  ...
  # PRIMARY NESTED TEST:
  # Does CEEMDAN add information after GARCH + HAR state are already present?
  c(
    "GARCH_HAR_CEEMDAN_CORR",
    "GARCH_HAR_CORR"
  ),

  # Does adding HAR to CEEMDAN correction improve it?
  c(
    "GARCH_HAR_CEEMDAN_CORR",
    "GARCH_CEEMDAN_CORR"
  ),

  # Context versus raw GARCH.
  c(
    "GARCH_HAR_CORR",
    "GARCH_11"
  ),
...
config_lines <- c(
  ...
  "",
  "GARCH_CAL:",
  "  trivial rolling multiplicative calibration c*GARCH, where",
  "  c = mean(actual/GARCH) on the purged historical training window.",
  "",
  "GARCH_HAR_CORR:",
  "  GARCH log-error correction using HAR state only.",
  "",
  "GARCH_CEEMDAN_CORR:",
  "  GARCH log-error correction using CEEMDAN state only.",
  "",
  "GARCH_HAR_CEEMDAN_CORR:",
  "  GARCH log-error correction using HAR + CEEMDAN state.",
  "",
  "PRIMARY IDENTIFICATION:",
  "  GARCH_HAR_CEEMDAN_CORR vs GARCH_HAR_CORR isolates incremental CEEMDAN.",
  "",
  "SECOND IDENTIFICATION:",
  "  GARCH_CEEMDAN_CORR vs GARCH_CAL asks whether CEEMDAN beats trivial calibration."
)
...
cat(
  ...
  "  1. GARCH_HAR_CEEMDAN_CORR vs GARCH_HAR_CORR  [clean nested CEEMDAN test]\n",
  "  2. GARCH_CEEMDAN_CORR vs GARCH_CAL            [CEEMDAN vs trivial calibration]\n",
  "  3. GARCH_CEEMDAN_CORR vs GARCH_HAR_CORR       [CEEMDAN state vs HAR state]\n",
  "  4. GARCH_CAL vs GARCH_11                      [how much simple calibration explains]\n",
  "  5. CEEMDAN_RIDGE vs HAR_RIDGE                 [direct-model incremental test]\n",
  "\n",
  sep = ""
)

cat(
  "For CEEMDAN to survive this experiment, the strongest evidence would be:\n",
  "  - GARCH_HAR_CEEMDAN_CORR has lower OOS QLIKE than GARCH_HAR_CORR,\n",
  "  - paired mean QLIKE difference is negative,\n",
  "  - block-bootstrap CI is below zero,\n",
  "  - the result is not explained by lambda sitting at the grid maximum,\n",
  "  - and performance is not driven by a handful of volatility episodes.\n\n",
  sep = ""
)
